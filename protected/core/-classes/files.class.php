...
class Files
{
	...
	public function unlinkFiles($files = [])
	{
		...
		if (!empty($files))
		{
			...
			{
				if ($file['file'] !== '' && file_exists(PATH_ROOT . $file['file']))
				{
					unlink(PATH_ROOT . $file['file']);

					Q("DELETE FROM `".$this->table."` WHERE `id`=?i LIMIT 1", array( $file['id'] ));
	...
	public function deleteFile($id = 0, $recursive = false)
	{
		...
		$cp = Q("SELECT `id`, `fid`, `file` FROM `".$this->table."` WHERE `id`=?i LIMIT 1", array( $id ))->all();
	...
	public function prepareName($name, $tmp_name) {
		...
		$arr = implode(".",$arr);
		//
		if ($this->prefix) $res = $this->prefix."_";
