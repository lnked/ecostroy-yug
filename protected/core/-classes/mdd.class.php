...
class MDD
{
	...
	private $meta_fields = array(
		"meta_title" 		=> array('f_name' => "META title", 			'f_sys_name' => "meta_title", 		'f_width' => 0,  'f_type' => "text", "ord" => "0", "in_list" => "0"),
		"meta_keywords" 	=> array('f_name' => "META keywords", 		'f_sys_name' => "meta_keywords", 	'f_width' => 0,  'f_type' => "text", "ord" => "0", "in_list" => "0"),
		"meta_description" 	=> array('f_name' => "META description",	'f_sys_name' => "meta_description", 'f_width' => 0,  'f_type' => "text", "ord" => "0", "in_list" => "0"),
		"meta_robots" 		=> array('f_name' => "META robots", 		'f_sys_name' => "meta_robots", 		'f_width' => 0,  'f_type' => "checkbox", "ord" => "0", "in_list" => "0")
	);

	private $index_fields = array(
	...
	{
		$result = [];
		$fields = Q("SELECT `id`, `module_id`, `f_name`, `f_sys_name`, `f_type`, `f_width`, `f_settings`, `f_index`, `f_unique`, `f_default`, `ord`, `in_list` FROM `".$this->table_fields."` WHERE `module_id`=?i GROUP BY `f_sys_name` ORDER BY `ord`", array( $module_id ))->all();
						...
						if ($ar['f_type'] == 'embedded' && isset($f_settings['f_module']))
						{
							...
							if (!empty($_temp))
							{
								foreach ($_temp as $_field)
								{
									...
									if (in_array($_field['f_sys_name'], $f_settings['f_fields']))
									{
										$_checked = true;
									}
	...
	public function createTable($id = 0)
	{
		...
		if ($this->existTable($table_name)) return "Таблица существует";
		...
		foreach($fields as $v)
		{
			...
			if ($v['f_sys_name'] == 'id') continue;
			$sql .= ", `".$v['f_sys_name']."` ". $type ."";
	   }
		
		$sql .= " , PRIMARY KEY (`id`)) ENGINE=" . $module['storage'] . " DEFAULT CHARSET=".$this->charset.";";
	...
	{
		if (!empty($name)) return $this->table_prefix . $name ;
		return '';
	}
	...
	public function updateTable($id = 0, $table_name = '')
	{
		if ($id !== 0)
		{
			...
			if ($table_name !== $old_name)
			{
				...
				if ($this->existTable($table_name)) return false;
