...
class Localization
{
    ...
    public function getDictionaryList($page = 0)
    {
        $list = array();
        $pager = array();
        ...
        $pager['all_items'] = Q("SELECT COUNT(`id`) AS `count` FROM `#".$this->db_dictionary."` WHERE `locale` LIKE 'ru' LIMIT 1")->row('count');
        ...
        $temp = Q("SELECT * FROM `#" . $this->db_dictionary . "` GROUP BY `key` ORDER BY `id` DESC LIMIT ?i, ?i", array( $pager['start_item'], $pager['limit'] ))->all('key');
