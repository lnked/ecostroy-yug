...
class file_driver
{
	...
	public function resize($settings = [], $insertData = true)
	{
		...
		if (!empty($settings))
		{
			...
			else
			{
				...
				$this->_resize($this->_temp['file'], $settings, $name);
			}
	...
	private function _resize($source = '', $settings = [], $name = '')
	{
		...
		if (file_exists($_source))
		{
			...
			foreach ($settings as $p)
			{
				...
				if ($_max > 0)
				{
					...
					if ($this->insertData)
					{
						$this->_insert($this->file_name, $this->file_path, filesize(PATH_ROOT . $this->file_path));
					}
