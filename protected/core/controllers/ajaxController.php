	...
	public function structure()
	{
		...
		if ($act == "tree")
		{
			...
			if ($mod_id) {
				...
				return json_encode($module->getModuleForStc());
			}
			else
				return json_encode($stc->getChilds( $_POST['pid'] ));
		}
		elseif ($act == "ajaxAddTemplate")
		{
			...
			if ( $name !== '' && $file !== '' )
			{
				Q("INSERT INTO `#__str_templates` SET `name`=?s, `sys_name`=?s on duplicate key update `sys_name`=?s", array( $name, $file, $file ));
				return json_encode( Q("SELECT * FROM `#__str_templates` WHERE `name`!='' AND `sys_name`!=''")->all());
			}
		}
