...
final class structureController extends cpLoader
{
	...
	public function index()
	{
		...
		if (in_array($this->method, array('edit')))
		{
			...
			{
				$info['cnt_page'][$key]['breadcrumbs'] = array();
			}
	...
	public function configure()
	{
		...
		{
			$_SESSION['configuration'] = json_decode(file_get_contents(ADMIN_PATH . '/json/config.json'), true);
		}

		$this->config = $_SESSION['configuration'];
	}

	private function crawler($branch = array(), $url = '/')
	{
		foreach ($branch as $tree)
		{
			$_url = $url;

			if (isset($tree['pid']) && $tree['pid'] != 0)
			{
				if ($_url !== '/')
				{
					$_url .= '/';
			...
			Q("UPDATE `#__str_structure` SET `url`=?s WHERE `id`=?i LIMIT 1", array( $_url, $tree['id'] ));
			
			if (!empty($tree['tree']))
			{
				$this->crawler($tree['tree'], $_url);
			}
		}
	}
	...
	private function makeUrl($id = 0)
	{
		...
		$structure = Q("SELECT `id`, `pid`, `locale`, `name`, `sys_name`, `url` FROM `#__str_structure` ORDER BY `pid`", array( $id ))->all();
		// $structure = $this->makeTree($structure, $pid);
		$structure = $this->makeTree($structure, 0);

		$this->crawler($structure);
	}
	...
	public function post()
	{
		...
		if ($action == "add")
		{
			if (!$stc_arr['name']) return false;
			if (!$stc_arr['sys_name']) return false;
