...
class Vote{
	var $Link_ID  = 0;
	var $Query_ID = 0;
	var $tab_theme = "#__vote_theme";
	var $tab_quest = "#__vote_questions";
	var $tab_result = "#__vote_result";
	var $use_coockie = 0; #0 не использовать куки
	var $coockie_life_time = 155520000; //Время жизни куков 60 * 60 * 24 * 30 * 6 = 155520000 - 0.5 года
	var $error = array(
		'err1'		=>		"empty",
		'err2'		=>		"Query error",
	...
	function add_vote($vote , $visible, $ord){
		if ($vote != ""){
			$this->mysql->query("INSERT INTO `".$this->tab_theme."` 
								SET `title`='".stripslashes($vote)."',`created`=NOW(), `visible`='".$visible."', `ord`='".$ord."' ");
			if ($this->mysql->last_id()) return 1;
			else return $this->error['err2'];
		}else return $this->error['err1'];
	}
	
	#Добавление вопроса
	function add_question($parent_id, $question , $ord , $visible){
		if ($question != ""){
			$this->mysql->query("INSERT INTO `".$this->tab_quest."` 
								SET `title` = '".$question."', `parent_id` = '".$parent_id."', `visible` = '".$visible."', `ord` = '".$ord."'  ");
			if (!$this->mysql->last_id()) return $this->error['err2'];
			else return $this->mysql->last_id();
		} else return 0;
	}
	...
	function get_vote(){
		...
		if ($this->mysql->nf()){
			...
			while($this->mysql->next_record()){
				if ($id != $this->mysql->f('vote_id')){ $id = $this->mysql->f('vote_id'); }
				$array[$id][] = array(
				//$array[] = array(
	...
	function pooling($vote_id,$pool_id){
		if ($vote_id){
			if ($pool_id != ""){
	...
	#Удаление вопроса
	function delete_question($quest_id){
		if ($quest_id != ""){
	...
	function update($value, $table){
		...
		if ($this->mysql->affected_rows()) return 1;
	}
	...
	function parser($result , $param){
		$result_table = array();
		if (!empty($result)){
			if ($param == 1) $status = $this->error['err3'];
			else $status = "0";
			foreach($result as $k => $v){
				...
				foreach($result as $key => $value){
					if ($value['vote_question_count'] == 0) $percent = 0;
					else $percent =($value['vote_question_count'] / $summ) * 100;
					$array_result[] = array(
