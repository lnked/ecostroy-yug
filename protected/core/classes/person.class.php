...
class Person
{
	var $user = array('id' => 0, 'gid' => 0, 'login' => "guest", 'name' => "Гость", 'admin_access' => 0); //	$user - define default user
	var $table = "#__usr_users";
	var $table_ai = "#__usr_add_info";
	var $table_af = "#__usr_add_fields";
	var $table_gr = "#__usr_groups";
	var $user_fields = array('id', 'gid', 'name', 'email', 'login', 'password', 'code', 'active', 'created', 'add_info', 'last_login', 'save', 'saving', 'comid', 'apply', 'action');
	...
	function getEmailsFromGroups($arr)
	{
		if (is_array($arr)) {
	...
	//	Show list of users
	function usersList($gid = 0, $pager = 10, $ord = "id", $ord_type = "DESC")
	{
	...
	//	AddUser
	function addUser($gid, $name, $email, $login, $password, $active = 0, $add_info = "")
	{
	...
	//	EditUser
	function editUser($id, $gid = 0, $name = '', $email = '', $login = '', $password = '', $active = 0, $add_info = "")
	{
	...
	//	Add group
	function addGroup($name, $description, $admin_access, $modules_access, $confirm)
	{
        $modules_access = '';
	...
	//	Add group
	function editGroup($id, $name, $description, $admin_access, $modules_access, $confirm)
	{
		$modules_access = '';
		if (isset($_POST['modules_access'] ) )
        {
	...
	function getGroupsList()
	{
		...
		foreach($data as &$item)
		{
			...
			if ($item['modules_access'] !== '')
            {
                ...
                {
					if (isset($modules[ $id ]['name'])) $str .= $modules[ $id ]['name'] . ', ';
				}
	...
	function additionsAdd($name, $sys_name, $field_type, $necess, $ord, $remote_table, $remote_field, $table_list)
	{
	...
	function additionsEdit($id, $name, $sys_name, $field_type, $necess, $ord, $remote_table, $remote_field, $table_list)
	{
		$sys_name = str_replace(array(' ', '-'), '_', $sys_name);
	...
	function registryUser( $gid )
	{
		...
		$name                        = __post("name");
		$_SESSION['registry_errors'] = array();
