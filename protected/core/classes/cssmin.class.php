...
class CSSmin
{
    ...
    public function run($css = '', $linebreak_pos = FALSE)
    {
        ...
        // Minify each chunk
        $_count = count($css_chunks);
        for ($i = 0, $n = $_count; $i < $n; $i++) {
    ...
    private function minify($css, $linebreak_pos)
    {
        ...
        $_count = count($this->preserved_tokens);
        for ($i = 0, $max = $_count; $i < $max; $i++) {
