...
class Meta extends MDD
{
	...
	public function __construct($module_id)
	{
		...
		//	Get module
		$this->module_info = $this->getModules(intval($module_id));
		...
		if (isset($_COOKIE['module_limit_' . $this->module_info['id']]))
		{
			$this->pager = $_COOKIE['module_limit_' . $this->module_info['id']];
		}
	...
	function filterModule($module_id = 0, $meta_module = [])
	{
		$filter = [];
		...
		{
			if (in_array($module['f_type'], array('select', 'multiselect', 'radio', 'checkbox')))
			{
	...
	function sortModule($module_id = 0, $meta_module = [])
	{
		return [];
		...
		foreach ($meta_module as $val)
		{
			if (in_array($val['f_type'], array(10, 11, 12)))
			{
				if ($val['f_table_dep'] != '' && $val['f_table_field'] != '')
				{
					$tmp = array(
						"name" 		=> $val['f_name'],
						"sys_name" 	=> $val['f_sys_name'],
						"list" 		=> $this->getTypeLists($val['f_table_dep'], $val['f_table_field']),
					);
					
					$sort[$val['f_sys_name']] = $tmp ;
				}
				...
				if ($val['f_table_list'])
				{
					$tmp = array(
						"name" 		=> $val['f_name'],
						"sys_name" 	=> $val['f_sys_name'],
						"list" 		=> $this->getTypeLists($val['f_table_list']),
					);
		...
		if (!empty($info['meta_sort']) && !empty($info['meta_item']))
		{
			foreach ($info['meta_item'] as $k => $v)
			{
				if (isset($info['meta_sort'][$v['f_sys_name']]))
				{
					$info['meta_item'][$k]['list'] = $info['meta_sort'][$v['f_sys_name']]['list'] ;
				}
			}
		}
		*/
	...
	function loadAdditions($module_id = 0, $meta_module)
	{
		$result = [];
	...
	function prepareFieldArr($module_fields)
	{
		$arr = [];
	...
	//	Get list of module for structure
	function getModuleForStc()
	{
		//	Get necessary fields
		$sql = "SELECT * FROM `".$this->table_fields."` WHERE `module_id`='".$this->module_info['id']."' ORDER BY `ord` ";
		
		//Q("SELECT * FROM `".$this->table_fields."` WHERE `module_id`='".$this->module_info['id']."' ORDER BY `ord` ")->all();

		$this->mysql->query($sql);
		$title_field = "";
		while($this->mysql->next_record() && $title_field == "")
		{
			if ($this->mysql->f("f_sys_name") != "id") $title_field = $this->mysql->f("f_sys_name");
		}
	...
	function metaEnable($module_id = '', $item_id = 0)
	{
		if (Q("SELECT COUNT(*) as `count` FROM `".$this->table_fields."` WHERE `f_type` LIKE 'meta' AND `module_id`=?i LIMIT 1 ", array($module_id))->row('count') > 0)
		{
			if ($item_id)
			{
				$meta = Q("SELECT `meta_title`, `meta_keywords`, `meta_description`, `meta_robots` FROM `#_mdd_" . $this->module_info['sys_name'] . "` WHERE `id`=?i LIMIT 1", array($item_id))->row();
	...
	{
		$temp = Q("SELECT `visible` FROM `".$this->table_main."` WHERE `id`=?i LIMIT 1", array( $id ))->row('visible');

		$visible = ($temp == 0) ? 1 : 0;

		Q("UPDATE `".$this->table_main."` SET `visible`=?i WHERE `id`=?i LIMIT 1", array( $visible, $id ));

		return true;
	}
	
	//
	function getChildsForStc()
	{
	...
	public function listModule($fields = [])
	{
		...
		foreach ($fields as $item)
		{
			if (isset($item['in_list']) && $item['in_list'] == 1 && $item['f_type'] !== 'meta')
			{
				...
				if (in_array($item['f_type'], array('select', 'radio', 'checkbox', 'multiselect')))
				{
		...
		foreach ($result as $key => $item)
		{
			foreach ($item as $name => $value)
			{
				...
				if ($field['f_sys_name'] !== '')
				{
					if (in_array($field['f_type'], array('select', 'radio')))
					{
	...
	function itemModule($item_id = 0, $fields, $base = '')
	{
		if ($item_id !== 0)
		{
			$sql = "SELECT ";
	...
	function getListInfo($arr = [])
	{
		foreach ($arr as $k => $item)
		{
			if (isset($item['f_type']))
			{
				...
				{
					$list = Q("SELECT `var`, `value`, `default` FROM `".$this->table_lists."` WHERE `list_name` LIKE '" . $item['f_settings']['f_table_list'] . "' ORDER BY `ord`")->all() ;
				...
				if (isset($item['f_settings']['f_table_name']) && $item['f_settings']['f_table_name'] !== '' && isset($item['f_settings']['f_table_field']) && $item['f_settings']['f_table_field'] !== '')
				{
					...
					else
					{
						...
						if ($this->existTable($item['f_settings']['f_table_name']))
						{
							...
							{
								$list = Q("SELECT `id` as `value`, `" . $item['f_settings']['f_table_field'] . "` as `var` FROM `" . $item['f_settings']['f_table_name'] . "` ")->all();
				...
				if (in_array($item['f_type'], array('file', 'image')))
				{
					...
					$arr[$k]['list'] = $_list;
					$arr[$k]['json_list'] = json_encode($arr[$k]['list'], 64 | 256);
				}
	...
	function is_req_table($table,$field)
	{
	...
	function addFiles($data, $group, $sys_name)
	{
	...
	public function updateData($module_id = 0, $item_id = 0)
	{
		...
		foreach ($fields as $k => $item)
		{
			if (in_array($item['f_sys_name'], array('id', 'gid', 'created', 'updated', 'module_id')))
			{
				continue;
			}
	...
	//	
	function tree_list_tree($a_list = [], $id=0, $all_tree)
	{
		if (!isset($a_list[$id]) || empty($a_list[$id])) return [];
				...
				if (in_array($f[$this->f_pid], $_SESSION['admin_struct']))
				{
					...
					if ($f['mod_id'] && in_array($f['id'], $_SESSION['admin_struct']))
					{
						...
						if (!isset($module->module_info['id']))
							...
							if (isset($f['a_tree'][0])) $f['childs'] = 1;
							else $f['childs'] = 0;
						}
	...
	//	Get
	function allTree($rootID=0,$all_tree=false)
	{
		
	}
	
	function treeModule_2($table,$field){
		$this->f_pid = $field;
		//
		$sql = "SELECT * FROM `".$table."` ORDER BY `".$this->f_pid."`,`ord`";
		$this->mysql->query($sql);
		$r = $this->mysql->Query_ID;
		if (!$this->mysql->nf()) return false;
