...
function __autoload($_class)
{
	...
	{
		if (file_exists(PATH_CORE.DS.'classes'.DS.$name . '.class.php'))
		{
			require_once PATH_CORE.DS.'classes'.DS.$name . '.class.php';
		}
	}
}
...
function url($path = '', $mask = array())
{
	//'link'      => ''.DS.$this->module_root.DS.$news['system']
	// exit(__debug($path));
}

function checkUrlLink($path = array(), $needle = array())
{
...
function __debug($var = array(), $name = '')
{
	echo '<pre>';
	if ($name) echo "$name:\n";
	print_r($var);
...
function __post($key = '', $post = array())
{
	$result = false;
