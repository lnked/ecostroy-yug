...
<div class="attachments js-fileupload" id="dropzone-{$name}"{if $action} data-action="/{$ADMIN_DIR}/{$action}"{/if}>
	...
	<div class="dz-message">
		...
		<span class="attachments__text">
			<i class="attachments__text__icon icon icon-attachment-alt"></i> Перетащите {if isset($settings.f_file_count) && $settings.f_file_count == 1}изображение{else}изображения{/if} сюда или <a class="attachments__text__link js-dropzone-target" id="dropzone-{$name}-target" tabindex="-1">выберите ...</a>
		</span>
				...
				<li class="attachments__pending__item" id="file_photo_{$e.id}">
					<div class="attachments__file clearfix">
						...
						<div class="attachments__file__control">
							<a href="" class="attachments__file__control__link" onclick="return Module.ajaxFileDelete({$e.id}, 'file_photo_{$e.id}');" title="Удалить файл" data-no-instant><i class="icon icon-delete"></i></a>
							<a href="" class="attachments__file__control__link" onclick="return editOrd({$e.id}, {$e.ord})" title="Редактировать порядок"><i class="icon icon-edit"></i></a>
							<a href="" class="attachments__file__control__link" onclick="return editVisible({$e.id}, {$e.visible})"><i class="icon icon-eye{if $e.visible == 0}-off{/if}" id="fvisible_{$e.id}"></i></a>
						</div>
...
<div class="js-dropzone-template dz-preview dz-file-preview" style="display: none;">
   	<li class="attachments__pending__item dz-details">
		<div class="attachments__file clearfix">
			...
			<!--
			<div class="attachments__file__control">
				<a href="" class="attachments__file__control__link"><i class="icon icon-delete"></i></a>
				<a href="" class="attachments__file__control__link"><i class="icon icon-edit"></i></a>
				<a href="" class="attachments__file__control__link"><i class="icon icon-eye"></i></a>
			</div>
			-->
