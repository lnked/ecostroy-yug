...
{if isset($list ) && !empty( $list )}
    {foreach from=$list item=li name=section}
        <div style="{if !$smarty.foreach.section.last}margin-bottom: 10px;{/if}clear: both;" class="clearfix" id="section-{$name}-item-{$li.value}">
            <a data-no-instant onclick="return removeSection('#section-{$name}-value', event, {$li.value}, '#section-{$name}-item-{$li.value}');" title="Удалить" class="icon icon-delete remove-trigger" style="float: left" href="#"></a>
