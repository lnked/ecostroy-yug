            ...
            {if !isset($structure_item.m_link)}
                ...
                <span class="structure__plus js-structure-controll">
                    ...
                    <span class="structure__control animate">
                        <a href="/{$ADMIN_DIR}/{if $structure_item.mod_id}meta/add/{$structure_item.mod_id}?backuri=/{$ADMIN_DIR}/structure/{else}structure/index/add/{$structure_item.id}/{/if}" class="structure__control__icon icon icon-file-plus" title="Добавить подраздел"></a>
                        <a href="/{$ADMIN_DIR}/{if isset($structure_item.mid )}meta/edit/{$structure_item.mid}/{$structure_item.id}?backuri=/{$ADMIN_DIR}/structure/{else}structure/index/edit/{$structure_item.id}/{/if}" class="structure__control__icon icon icon-edit" title="Редактировать раздел"></a>
                        <a href="#" onclick="ajax_vis_toggle(this, {$structure_item.id},{if isset($structure_item.mid )}{$structure_item.mid}{else}0{/if});return false;" class="structure__control__icon icon icon-eye{if $structure_item.visible == 0}-off{/if}" title="Отображение страницы"></a>
                        {if $structure_item.id !== '1'}
                        <a href="/{$ADMIN_DIR}/{if isset($structure_item.mid )}meta/del/{$structure_item.mid}/{$structure_item.id}?backuri=/{$ADMIN_DIR}/structure/{else}structure/index/del/{$structure_item.id}/{/if}" class="structure__control__icon icon icon-delete" title="Удалить раздел" onclick="return cp.dialog('Вы действительно хотите удалить раздел?');" data-no-instant></a>
                        {/if}
                    </span>
                </span>
            {/if}
...
<div class="form__row clearfix">
	...
	<div class="form_column">
		<a href="#" class="button button-link"><i class="icon icon-plus-square"></i>Добавить</a>		
	</div>
</div>
...
<form action="" class="form">
	<div class="form__row clearfix">
		...
		<div class="form_column">
			<button class="button button-blue"><i class="icon icon-save"></i>Сохранить</button>
		</div>
	</div>
</form>
*}