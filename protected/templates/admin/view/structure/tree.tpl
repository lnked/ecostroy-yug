...
{foreach item=item from=$a_tree}	
	<li class="clearfix">
		{if !isset($item.m_link )}
            ...
            <div class="icons">
                {* add *}
                <a href="/{$ADMIN_DIR}/{if $item.mod_id}meta/add/{$item.mod_id}?backuri=/{$ADMIN_DIR}/structure/{else}structure/index/add/{$item.id}/{/if}" class="icon icon-file-plus" title="Добавить подраздел"></a>
                
                {* edit *}
                <a href="/{$ADMIN_DIR}/{if isset($item.mid )}meta/edit/{$item.mid}/{$item.id}?backuri=/{$ADMIN_DIR}/structure/{else}structure/index/edit/{$item.id}/{/if}" class="icon icon-edit" title="Редактировать раздел"></a>
                
                {* visible *}
                <a href="#" onclick="ajax_vis_toggle(this, {$item.id},{if isset($item.mid )}{$item.mid}{else}0{/if});return false;" class="icon icon-eye{if $item.visible == 0}-off{/if}" title="Отображение страницы"></a>
                
                {* del *}
                {if $item.id !== '1'}
                    <a href="/{$ADMIN_DIR}/{if isset($item.mid )}meta/del/{$item.mid}/{$item.id}?backuri=/{$ADMIN_DIR}/structure/{else}structure/index/del/{$item.id}/{/if}" class="icon icon-delete" title="Удалить раздел" onclick="return cp.dialog('Вы действительно хотите удалить раздел?');" data-no-instant></a>
                {/if}
            </div>
		{/if}
