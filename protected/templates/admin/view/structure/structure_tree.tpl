				...
				*}
				...
				<span class="structure__plus js-structure-controll">
					...
					<span class="structure__control animate">
						<a href="/{$ADMIN_DIR}/{if $item.mod_id}meta/add/{$item.mod_id}?backuri=/{$ADMIN_DIR}/structure/{else}structure/index/add/{$item.id}/{/if}" class="structure__control__icon icon icon-file-plus" title="Добавить подраздел"></a>
						<a href="/{$ADMIN_DIR}/{if isset($item.mid )}meta/edit/{$item.mid}/{$item.id}?backuri=/{$ADMIN_DIR}/structure/{else}structure/index/edit/{$item.id}/{/if}" class="structure__control__icon icon icon-edit" title="Редактировать раздел"></a>
						<a href="#" onclick="ajax_vis_toggle(this, {$item.id},{if isset($item.mid )}{$item.mid}{else}0{/if});return false;" class="structure__control__icon icon icon-eye{if $item.visible == 0}-off{/if}" title="Отображение страницы"></a>
						{if $item.id !== '1'}
						<a href="/{$ADMIN_DIR}/{if isset($item.mid )}meta/del/{$item.mid}/{$item.id}?backuri=/{$ADMIN_DIR}/structure/{else}structure/index/del/{$item.id}/{/if}" class="structure__control__icon icon icon-delete" title="Удалить раздел" onclick="return cp.dialog('Вы действительно хотите удалить раздел?');" data-no-instant></a>
						{/if}
					</span>
				</span>
			{/if}
