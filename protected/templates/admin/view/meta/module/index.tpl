    ...
    <div class="button-container clearfix">
                    ...
                    <div class="fieldset-content">
                        {if isset($item.modules ) && !empty( $item.modules )}
                            {foreach item=i from=$item.modules key=id}
                                {if is_array( $i )}
                                    <a href="{$base_path}/module/list/{$id}" class="button button-{cycle values='red,blue,green,purple,orange,pink' name='color'}"><i class="icon icon-link"></i>{$i.name} <span class="baloon">{$i.count}</span></a>
                                {/if}
                            {/foreach}
                        {/if}
                    </div>
        ...
        {if isset($modules) && !empty($modules)}
            ...
            {foreach item=item from=$modules name=i}
                <a href="{$base_path}/module/list/{$item.id}" class="button button-{cycle values='red,blue,green,purple,orange,pink' name='color'}"><i class="icon icon-link"></i>{$item.name} <span class="baloon">{$item.count}</span></a>
            {/foreach}
        {/if}
    </div>
{else}
