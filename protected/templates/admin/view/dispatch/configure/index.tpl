...
<table class="table">
    ...
    {if isset($list_dispatch ) && !empty( $list_dispatch )}
        ...
        <tr>
            ...
            <td class="tac">
                <a href="/{$ADMIN_DIR}/dispatch/edit/{$item.id}/" class="icon icon-edit" title="Редактировать"></a>
                <a href="/{$ADMIN_DIR}/dispatch/del/{$item.id}/" class="icon icon-delete remove-trigger" title="Удалить" onclick="return cp.dialog('Вы действительно хотите удалить форму?');" data-no-instant></a>
            </td>
        </tr>
        {/foreach}
    {else}
