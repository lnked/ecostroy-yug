...
<table class="table">
    ...
    {foreach from=$exchange_files item=item name=exchange}
        <tr>
            <td><i class="icon icon-file"></i> {$item.file}</td>
            <td>
                <form action="/{$ADMIN_DIR}/import" method="GET">
                    ...
                    <button type="submit" class="button"><i class="icon icon-upload"></i> Импортировать</button>
                    {/if}
                </form>
            </td>
            <td>{$item.size}</td>
            <td>{$item.time}</td>
            <td class="tac">
                <a href="#" class="icon icon-delete remove-trigger" title="Удалить" onclick="return cp.dialog('Вы действительно хотите удалить файл?')" data-no-instant></a>
            </td>
        </tr>
    {/foreach}
    </tbody>
...
<form method="post" enctype="multipart/form-data">
    {if $type == '1c'}
        ...
        <button type="submit" class="button button-green"><i class="icon icon-file"></i> Загрузить</button>
    {elseif $type == 'xls'}
        <div class="apply notice">
            Последний обмен данными был выполен<br><b>12.12.2013</b> 23:48
        </div>
        ...
        <button type="submit" class="button button-green"><i class="icon icon-check-square"></i> Импортировать файл</button>
    {/if}
</form>
{/if}
*}
