	...
	<table class="table">
	    ...
	    {foreach from=$profiles item=item name=profiles}
	        <tr>
	            ...
	            <td class="tac">
	                <a href="./del-field/{$item.id}/" class="icon icon-delete remove-trigger" title="Удалить" onclick="return cp.dialog('Вы действительно хотите удалить поле?')" data-no-instant></a>
	            </td>
	        </tr>
	    {/foreach}
	    </tbody>
...
{if !$module_id nocache}
	...
	<div class="button-container clearfix">
        <button class="button button-green" type="submit" data-no-instant><i class="icon icon-save"></i>Добавить профиль</button>
    </div>
{else nocache}
