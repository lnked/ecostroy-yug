...
<table class="table">
    ...
    {if isset($log_list ) && !empty( $log_list ) nocache}
        ...
        <tr>
            ...
            <td>{$item.date}</td>
            <td class="tac">
                <a href="{$base_path}/logs/del/{$item.id}/{if $back_to_page}?page={$back_to_page}{/if}" class="icon icon-center icon-delete" title="Удалить" onclick="return cp.dialog('Вы действительно хотите удалить запись?');" data-no-instant></a>
            </td>
        </tr>
        {/foreach}
    {else}
